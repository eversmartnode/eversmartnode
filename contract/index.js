...
class ContractContext {
    ...
    // Update HotPocket's live peer set through the contract control channel.
    // hpcore 0.6.4 requires BOTH peer_changeset fields to be arrays. EverPocket
    // exposes a wider wrapper type for `toRemove` (string[] | string | null), so
    // an older caller can leak the legacy "*" overwrite sentinel into this API.
    // hpcore then rejects the entire changeset with:
    //   Extract peers: Invalid array field: remove
    // Normalize at the wire boundary. "*" is deliberately downgraded to an
    // additive-safe update: AutoCluster performs explicit stale-peer removals and
    // must never tear down every live consensus session in one operation.
    updatePeers(toAdd, toRemove = []) {
        if (!this.#controlChannel)
            throw "Control channel is not initialized.";

        const peerArray = (value, allowSingle = false) => {
            let list = [];
            if (Array.isArray(value)) list = value;
            else if (allowSingle && typeof value === 'string' && value.trim() && value.trim() !== '*') list = [value];
            const out = [];
            for (const v of list) {
                if (typeof v !== 'string') continue;
                const peer = v.trim();
                if (!peer || peer === '*') continue;
                // Keep the wire boundary strict, but do not invent an IP-only
                // restriction. HotPocket deployments commonly use stable DNS peers.
                // Accept a canonical hostname/IPv4 : port or bracketed IPv6 : port;
                // reject placeholders such as undefined:undefined and malformed
                // host:port:port values before they reach hpcore.
                let host = '', portText = '';
                if (peer.startsWith('[')) {
                    const end = peer.indexOf(']');
                    if (end < 2 || peer[end + 1] !== ':') throw new Error(`Invalid live HotPocket peer endpoint: ${peer}`);
                    host = peer.slice(1, end);
                    portText = peer.slice(end + 2);
                } else {
                    const split = peer.lastIndexOf(':');
                    if (split <= 0) throw new Error(`Invalid live HotPocket peer endpoint: ${peer}`);
                    host = peer.slice(0, split).trim();
                    portText = peer.slice(split + 1).trim();
                    if (host.includes(':')) throw new Error(`Invalid live HotPocket peer endpoint: ${peer}`);
                }
                const port = Number(portText);
                const hostOk = /^(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)(?:\.(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?))*\.?$/.test(host)
                    || /^(?:\d{1,3}\.){3}\d{1,3}$/.test(host)
                    || (peer.startsWith('[') && /^[0-9A-Fa-f:]+$/.test(host));
                if (!hostOk || /^(?:undefined|null)$/i.test(host) || !Number.isInteger(port) || port < 1 || port > 65535)
                    throw new Error(`Invalid live HotPocket peer endpoint: ${peer}`);
                ...
                const canonical = peer.startsWith('[') ? `[${host}]:${port}` : `${host}:${port}`;
        ...
        const msg = {
            ...
            add: peerArray(toAdd, true),
            remove: peerArray(toRemove, true)
        };
        if (!msg.add.length && !msg.remove.length) return Promise.resolve();
        console.log(`AutoCluster: HOTPOCKET CONTROL peer_changeset add=[${msg.add.join(', ')}] remove=[${msg.remove.join(', ')}]`);
        return this.#controlChannel.send(msg);
    }
}
...
class UnlCollection {
    ...
    onMessage(callback) {
        ...
        this.#channel.consume((publicKey, msg) => {
            // In public NPL mode HotPocket can deliver messages from connected
            // non-UNL nodes. Preserve the authenticated transport public key even
            // when that sender is not present in this.nodes so applications can
            // safely identify bootstrap candidates without a second HPWS session.
            const sender = this.nodes[publicKey] || { publicKey, activeOn: null, external: true };
            this.#pendingTasks.push((0,common.invokeCallback)(callback, sender, msg));
        });
    }
...
// Stable EverAdmin loader. Keep this file fixed; application updates are loaded
// from consensus state/contract-versions and can be deactivated without changing hp.cfg.
(() => {
...
const PRIMARY_BASE_APP = path.resolve(__dirname, 'sidedish.js');
const LEGACY_BASE_APP = path.resolve(__dirname, 'app.js');
const BASE_APP = fs.existsSync(PRIMARY_BASE_APP) ? PRIMARY_BASE_APP : LEGACY_BASE_APP;

function safeVersionId(value) {
  const id = String(value || '').trim();
  if (!/^[a-zA-Z0-9._-]{3,160}$/.test(id) || id.includes('..')) return null;
  return id;
}

function safeEntry(value) {
  const rel = String(value || 'sidedish.js').trim().replace(/\\/g, '/').replace(/^\/+/, '');
...
function resolveActiveApp() {
  ...
  const activeFile = path.resolve(stateRoot, 'contract-versions', 'active-version.json');
  if (!fs.existsSync(activeFile)) return { file: BASE_APP, source: 'base' };

  try {
    const active = JSON.parse(fs.readFileSync(activeFile, 'utf8'));
    const versionId = safeVersionId(active.versionId);
    const entryFile = safeEntry(active.entryFile || 'sidedish.js');
    if (!versionId || !entryFile) throw new Error('Invalid active version marker.');
    const root = path.resolve(stateRoot, 'contract-versions', versionId);
    const file = path.resolve(root, entryFile);
    if (!file.startsWith(root + path.sep)) throw new Error('Active version path escaped version root.');
    if (!fs.existsSync(file) || !fs.statSync(file).isFile()) throw new Error('Active version entry is missing.');
    return { file, source: versionId };
  } catch (err) {
    console.log(`EverAdmin loader: active version marker rejected: ${err.message}. Falling back to base app.`);
    return { file: BASE_APP, source: 'base-fallback' };
  }
}

function loadApp() {
  const selected = resolveActiveApp();
  try {
    delete require.cache[require.resolve(selected.file)];
    const mod = require(selected.file);
    const handler = typeof mod === 'function' ? mod : mod && mod.contract;
    if (typeof handler !== 'function') throw new Error('Application module must export contract(ctx, runtime) or a function.');
    return { handler, selected };
  } catch (err) {
    if (selected.file === BASE_APP) throw err;
    console.log(`EverAdmin loader: failed to load ${selected.source}: ${err.message}. Falling back to base app.`);
    delete require.cache[require.resolve(BASE_APP)];
    const mod = require(BASE_APP);
    const handler = typeof mod === 'function' ? mod : mod && mod.contract;
    if (typeof handler !== 'function') throw new Error('Base application module is invalid.');
    return { handler, selected: { file: BASE_APP, source: 'base-fallback' } };
  }
}

async function contract(ctx) {
  const loaded = loadApp();
  await loaded.handler(ctx, { bson, loaderSource: loaded.selected.source });
}
